			...
			if ($.fn.mColorPicker.init.replace == '[type=color]') {
				...
				stats.updateAccordion = function() {
					...
					stats.filter = function(p) {
						...
						title = "Filter",
						viewTypes = {"area":"","map":"","bubble":"","nationality":""}; // the list of possible views
						...
						.on("click",function(){bst.stats.showData(filter.data,this)})
						...
						filter.update = function(v, upv) {
							...
							filter.query = filter.prev ? JSON.parse(JSON.stringify(filter.prev.query)) : {};
							...
							// loadLocale(filter.query, function(r) {
								...
								load(stats.url, "action=query&q="+JSON.stringify(filter.query), function(r) {
								...
								load(stats.url, "action=query&q="+JSON.stringify(queryNull), function(r) {
							...
							filter.view = function(type, properties) {
								...
								switch(type) {
									...
									// BAD! TO IMPROVE! not a proer view, an exception, just for nationality
									case "nationality" :
									...
									view.updateView = function() { 
										...
										// listeners
										view.updated = function(d) {
											filter.query[filter.property] = {"$gte":d.range[0],"$lte":d.range[1]};
											filter.queryRender.attr("value",JSON.stringify(filter.query))
											filter.update(	filter.query[filter.property], true)
											filter.updated(filter);				
										};

										view.updateView = function() { view.update( bst.data.time(filter.data, filter.property) ,filter.color) }
										...
										view.updateView = function() {
											...
											else view.update( bst.data.countValues(filter.data, filter.property),filter.color )
										}
										...
										if (properties)
										view.updateView = function() {
											view.update( bst.data.geoCount(filter.data, properties, filter.property),filter.color )
										}
										else

										view.updateView = function() {

											view.update( bst.data.geo(filter.data, filter.property) ,filter.color)
										}			
							...
							bst.stats.showData = function(data,event,title) {
								...
								$(table[0][0]).dataTable( {
									"aaData": bst.data.table(data),
									"bProcessing": true,
									"aoColumnDefs": [ 
									{
										"fnRender": function ( oObj ) {
											return "<a href='http://e-enlightenment.com/item/"+oObj.aData[1]+"' target='blank'>"+oObj.aData[1]+"</a>";
										},
										"aTargets": [ 1 ]
									}
									],
